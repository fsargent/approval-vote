...
class CvrParser:
    ...
    def parse_xml_file(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Handle namespace
            ns = {'cvr': 'http://tempuri.org/CVRDesign.xsd'} if root.tag.startswith('{') else {}
            ...
            for contest_elem in findall_with_ns(contests_elem, 'Contest'):
                ...
                undervotes = int(undervotes_elem.text) if undervotes_elem is not None else 0
    ...
    def flush_batch(self) -> None:
        ...
        try:
            ...
            for ballot_record in self.ballot_batch:
                ...
                result = conn.execute("SELECT id FROM cvr_ballots WHERE cvr_guid = ?", (cvr_guid,)).fetchone()
                if result:
            ...
            for i, (ballot_index, contest_name, contest_id, undervotes) in enumerate(self.contest_batch):
                ...
                if cvr_guid in guid_to_ballot_id:
                    ...
                    if not existing:
                        updated_contest_batch.append((actual_ballot_id, contest_name, contest_id, undervotes))
            ...
            if updated_contest_batch:
                ...
                for i, (ballot_id, contest_name, contest_id, undervotes) in enumerate(updated_contest_batch):
                    result = conn.execute(
                        "SELECT id FROM cvr_contests WHERE ballot_id = ? AND contest_id = ?",
                ...
                for orig_contest_index, candidate_name, candidate_id, selection_value in self.selection_batch:
                    if orig_contest_index in contest_id_mapping:
                        ...
                        if new_contest_index in contest_records:
                            actual_contest_record_id = contest_records[new_contest_index]
                            updated_selection_batch.append((actual_contest_record_id, candidate_name, candidate_id, selection_value))
    ...
    def show_summary(self) -> None:
        ...
        selection_count = conn.execute("SELECT COUNT(*) FROM cvr_selections").fetchone()[0]
        ...
        print(f"\nTOP 10 PRECINCTS")
        print("-"*30)
        top_precincts = sorted(self.stats['precincts'].items(), key=lambda x: x[1], reverse=True)[:10]
...
@click.option('--data-dir', '-d', type=click.Path(exists=True, path_type=Path), 
              default=Path('data'), help='Directory containing CVR XML files')
@click.option('--output', '-o', type=click.Path(path_type=Path), 
              default=Path('cvr-data.sqlite3'), help='Output SQLite database file')
@click.option('--batch-size', '-b', type=int, default=5000, 
...
def main(data_dir: Path, output: Path, batch_size: int, verbose: bool):
    ...
    logger.info(f"Starting CVR parsing...")
